...
void absDiffNorm::computeRecMatrix()
{
    ...
    */
    ...
    for (int j=0; j<=N/(M-1); j++)
    {
        for (int i=0; i<totNumRecSteps; i++)
        {
            ...
            for (int k=i; k<i+(M-1); k++)
            {
                ...
                {
                    FatalError
                        << "absDiffNorm: Unknown field type " << fieldType_
                        << abort(FatalError);
                }

                recurrenceMatrix[ti][tj] = normIJ;

                if (normIJ > absDiffNormIJ)
                {
                    absDiffNormIJ = normIJ;
                }
                // end
            }
        }
    ...
    if(normConstant_ > 0.0) absDiffNormIJ = normConstant_;
    ...
    for(label ti=0;ti<totNumRecSteps;ti++)
    {
        for(label tj=0;tj<totNumRecSteps;tj++)
        {
            ...
            recurrenceMatrix[ti][tj] /= absDiffNormIJ;
...
scalar absDiffNorm::normVSF(label ti, label tj)
{
    ...
    dimensionedScalar tNorm( fvc::domainIntegrate( mag( t1 - t2 ) ) );
